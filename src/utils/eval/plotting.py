...
def generate_improvement_ratio_plot(
    ...
    if len(improvements) < 2:
        print("Need at least 2 methods with both NoInstruct/Instruct for improvement plot")
        return None

    # Also calculate for valid-only if provided
    valid_improvements = None
    if valid_only_scores:
        valid_improvements = calculate_improvement_delta(valid_only_scores, methods)  # noqa: F841
...
def generate_radar_chart_area_based(
    ...
    print(f"\n{'='*60}")
    print("INSTRUCTION ALIGNMENT EFFICIENCY (Average Radius)")
    print(f"{'='*60}")
