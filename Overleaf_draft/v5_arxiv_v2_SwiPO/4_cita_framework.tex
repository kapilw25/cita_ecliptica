...
This explicit conditioning is the key departure from standard DPO~\cite{rafailov2023direct}: rather than learning one implicit preference regime, SwiPO must fit \textbf{multiple co-existing instruction-conditioned regimes}.
...
We tune SwiPO with \textbf{Optuna}~\cite{akiba2019optuna} using the \textbf{TPE} sampler for \textbf{12 trials}, following recent HPO guidance~\cite{liang2023holistic}.
A consistent trend emerges: \textbf{SwiPO\_Instruct prefers a $\sim$50\% lower learning rate} than SwiPO\_NoInstruct.
