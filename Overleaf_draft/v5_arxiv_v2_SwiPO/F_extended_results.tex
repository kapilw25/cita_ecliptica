...
\textbf{SwiPO\_Instruct requires $\sim$50\% lower learning rate} than SwiPO\_NoInstruct.
...
The accuracy gap of $\sim$3 percentage points reflects a fundamental trade-off: DPO optimizes directly for pairwise preference discrimination, while SwiPO's unified loss includes a KL anchor term that prioritizes stability over raw preference fitting.
...
However, for SwiPO, this pattern inverts: NoInstruct (0.893) slightly outperforms Instruct (0.890).
...
Despite SwiPO's lower accuracy, it achieves substantially higher reward margins (Figure~\ref{fig:training_margins}).
...
\caption{\textbf{Reward margins.} SwiPO\_Instruct (7.5) $>$ SwiPO\_NoInstruct (7.2) $>$ DPO ($\sim$6.0). Larger margins indicate stronger separation between preferred vs.\ dispreferred responses under the instruction-conditioned preference relation.}
...
Figure~\ref{fig:isd} shows that \textsc{DPO}\_Instruct (0.389) and SwiPO\_Instruct (0.367) attain the strongest instruction awareness.

\begin{figure*}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{figures/evaluation/isd_comparison.pdf}
\caption{\textbf{ECLIPTICA (ISD) results.} \textsc{DPO}\_Instruct (0.389) and SwiPO\_Instruct (0.367) show the highest instruction awareness on the controlled instruction-switch benchmark.}
\label{fig:isd}
\vspace{-3mm}
\end{figure*}
...
\textbf{Second,} SwiPO remains highly competitive on ISD while also improving across \emph{other} benchmarks, suggesting it preserves \textbf{multi-axis switchability} instead of over-specializing to one instruction family.
...
\paragraph{Why SwiPO\_NoInstruct performs worst.}
SwiPO\_NoInstruct has the most negative score ($-0.040$), despite SwiPO\_Instruct achieving the best positive score.
This large gap ($\Delta = 0.053$) demonstrates that SwiPO's instruction channel provides \textbf{substantial calibration leverage} when present, but the NoInstruct variant inherits DPO's baseline miscalibration (since SwiPO stacks on DPO).
Without the instruction channel to modulate epistemic stance, SwiPO\_NoInstruct defaults to the DPO checkpoint's behavior, which lacks explicit calibration conditioning.
...
\caption{\textbf{Length control.} SwiPO\_Instruct (1.14) shows the strongest adaptation ratio (detailed/concise word count), indicating reliable compliance with explicit verbosity contracts.}
...
\vspace{-0.2em}
\noindent\textbf{Interpretation.}
SwiPO\_Instruct provides the strongest switching signal on this axis, supporting the claim that the instruction channel can enforce \textbf{measurable policy constraints} rather than cosmetic variation.
...
The ordering (SwiPO $>$ \textsc{DPO} $>$ \textsc{GRPO} $>$ \textsc{PPO}) suggests that preference-based methods may capture length constraints marginally better than pure RL methods, but the differences are small relative to the target gap.
...
\caption{\textbf{AQI.} SwiPO\_Instruct (55.0) achieves the highest AQI. PPO variants (40.8--43.5) outperform DPO (11.8--18.0) on cluster separation.}
\label{fig:aqi}
\vspace{-3mm}
\end{figure*}
...
\textbf{First,} SwiPO\_Instruct achieves the highest AQI, consistent with the hypothesis that instruction-conditioning plus stability anchoring can improve \textbf{axiom-level coherence}.
...
The ordering from lowest to highest is: \textsc{DPO}\_Instruct (11.8) $<$ \textsc{DPO}\_NoInstruct (18.0) $<$ \textsc{GRPO}\_NoInstruct (24.6) $<$ SwiPO\_NoInstruct (28.6) $<$ \textsc{GRPO}\_Instruct (31.5) $<$ \textsc{PPO}\_NoInstruct (40.8) $<$ \textsc{PPO}\_Instruct (43.5) $<$ SwiPO\_Instruct (55.0).
...
In contrast, online RL methods (PPO, GRPO) receive reward feedback that can shape global response distributions, and SwiPO's KL anchor explicitly prevents collapse to any single axis.
The result is that methods with regularization or online feedback achieve better axiom-level cluster separation.

\paragraph{SwiPO\_Instruct's substantial lead.}
SwiPO\_Instruct achieves 55.0 AQI, a \textbf{26\% improvement} over the second-best model (\textsc{PPO}\_Instruct at 43.5).
This result supports the hypothesis that SwiPO's design---combining preference learning with stability anchoring---produces \textbf{globally coherent} alignment rather than axis-specific overfitting.
...
SwiPO should produce \textbf{broad, reliable instruction sensitivity} without relying on LLM judges.
...
\textbf{SwiPO\_Instruct is the only method that is consistently strong across heterogeneous control axes}
...
Figure~\ref{fig:heatmap} presents an 8$\times$5 matrix where rows are model variants (DPO\_NI, DPO\_I, PPO\_NI, PPO\_I, GRPO\_NI, GRPO\_I, SwiPO\_NI, SwiPO\_I) and columns are benchmarks (ECLIPTICA $M_1$, TruthfulQA $M_2$, Cond.\ Safety $M_3$, Length Ctrl $M_4$, LITMUS/AQI $M_5$).
...
\paragraph{Row-wise analysis: SwiPO\_I dominates.}
Scanning horizontally across the SwiPO\_I row reveals predominantly green cells, indicating best or near-best performance on 4 of 5 benchmarks.
...
This visual pattern supports the claim that SwiPO achieves \textbf{balanced multi-axis switching} rather than specializing to a single dimension.
...
Only two cells (GRPO\_I: 0.011, SwiPO\_I: 0.013) are positive; all others are negative.
...
For TruthfulQA, SwiPO\_I (0.013 $\pm$0.19) and GRPO\_I (0.011 $\pm$0.27) have wide, overlapping intervals, indicating that the difference between them is \textbf{not statistically significant}.
...
SwiPO exhibits substantially stronger instruction sensitivity than DPO on this benchmark (positive adaptation vs.\ near-zero change), supporting the claim that SwiPO internalizes instruction-conditioned epistemic posture.

\paragraph{\textbf{Margins separate switching from static preference fitting.}}
Despite similar preference accuracy ($\sim$89\%), SwiPO attains higher reward margins than DPO.
We interpret this as evidence that SwiPO learns a \textbf{more separated instruction-conditioned preference geometry}.
...
SwiPO is less extreme on some single-axis separations, but more \textbf{uniformly switchable} across axes.
...
SwiPO enables \textbf{runtime-controllable alignment} that generalizes across multiple policy contracts,
