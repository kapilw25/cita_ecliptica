...
ECLIPTICA is a factorial grid of \textbf{300 prompts} (taken from \citep{bai2022training}) and \textbf{10 alignment instruction types}, yielding
...
The public release extends the instruction inventory from 10 to \textbf{100 types}, organised as 10 axes with 10 settings each (stance, constraint posture, delivery style, epistemic stance, verbosity and structure, audience, persona, action orientation, regional lens, output format), and crosses them with the same 300 prompts: \(300 \times 100 = 30{,}000\) prompt-held-constant cases. The 90 added types are hand-designed as a taxonomy (no judge models), each with three phrasings and three expected characteristics; they widen the instruction space for further research and \emph{have not been scored by any model in this paper}. Every experiment below uses the original 10-type subset, shipped verbatim as the \texttt{eval\_3k} configuration of the same dataset.
...
\midrule
Instruction generator & GPT-4 / GPT-4o-family & \cite{openai2023gpt4,openai2024gpt4o} \\ \hline
Instruction generator & Gemini 1.5 Pro & \cite{reid2024gemini15} \\ \hline
Instruction generator & Claude-family (system/model cards) & \cite{anthropic2023claude2,anthropic2024claude3addendum} \\ \hline
Instruction generator & Llama-3.1-Instruct (via Llama-3 report) & \cite{meta2024llama3} \\ \hline
