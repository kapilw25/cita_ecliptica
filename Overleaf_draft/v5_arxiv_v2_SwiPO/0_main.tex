...
\title{ECLIPTICA: A Framework for Switchable LLM Alignment via SwiPO, Switchable Preference Optimization}
...
{\small $^{1}$San Jos\'e State University, USA \quad $^{2}$UC Berkeley School of Information, USA \quad $^{3}$Meta, USA \quad $^{4}$Apple, USA \quad $^{5}$Pragya Lab, BITS Pilani Goa, India} \\[0.3em]
...
We introduce \textbf{SwiPO} (\textbf{Swi}tchable \textbf{P}reference \textbf{O}ptimization), combining supervised fine-tuning with contrastive preference optimization under an \emph{explicit, mandatory} \textbf{geometric anchor} to a reference model:
\(
...
\)
, yielding \textbf{a stable Riemannian chart.} SwiPO keeps instruction updates within a \textbf{shared manifold neighborhood} of the reference policy, so regimes stay \textbf{nearby and traversable}—enabling \textbf{stable switching} over superficial variation.
...
To separate \textbf{policy switching} from standard instruction following, we introduce the \textbf{ECLIPTICA benchmark}: $3{,}000$ controlled test cases ($300$ prompts $\times$ $10$ instruction types) in which the \emph{user request is held fixed} and only the alignment instruction varies, released as \textbf{ECLIPTICA-30k} ($300$ prompts $\times$ $100$ instruction types), of which the ten evaluated types form the $3{,}000$-case subset. On \textbf{Llama-3.1-8B} across five suites (ECLIPTICA, TruthfulQA, Conditional Safety, Length Control, LITMUS), \textbf{SwiPO attains \colorbox{bg-green}{86.7\%} instruction-alignment efficiency}, exceeding DPO (56.1\%; +30.6 pp), GRPO (36.1\%; +50.6 pp), and PPO (20.4\%; +66.3 pp). Together, \textbf{ECLIPTICA + SwiPO} move alignment beyond \emph{one-policy-per-checkpoint} toward \textbf{switchable, instruction-governed behavior} aligned with modern deployment and agentic settings. \href{https://github.com/kapilw25/cita_ecliptica}{Code} \& \href{https://huggingface.co/datasets/anonymousML123/ECLIPTICA-30k}{Dataset}
...
\textbf{SwiPO} makes switching \emph{reliable} via contrastive preference learning \emph{with a \textbf{mandatory} KL anchor} (stability, not prompt tricks).
...
SwiPO instantiates ECLIPTICA with a \textbf{single objective} coupling quality, preference contrast, and stability:
\vspace{-1em}
\[
\boxed{
\begin{aligned}
...
\textbf{SwiPO reaches \colorbox{bg-green}{86.7\%} instruction-alignment efficiency}—beating DPO (56.1\%, +30.6 pp), GRPO (36.1\%, +50.6 pp), PPO (20.4\%, +66.3 pp).

\item[\faTools] \textbf{\textit{Practical upshot}}:
ECLIPTICA + SwiPO move from \textbf{one-policy-per-checkpoint} to \textbf{switchable, instruction-governed alignment}:
...
\input{A_related_and_derivation}  % C. Related Works + SwiPO Loss Derivation (math first)
