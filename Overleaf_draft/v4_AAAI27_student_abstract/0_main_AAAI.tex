...
RLHF, DPO \cite{rafailov2023direct}, and their variants fix one policy per checkpoint, yet each agent role needs its own safety threshold, tone, and disclosure norms. We instead pass an alignment instruction $I$ next to the user prompt $X$. The same $X$ must then yield different aligned behaviors only because $I$ changed, and the switch must be clean, with no repetition, prompt leakage, incoherence, or unsafe drift (Figure~\ref{fig:teaser}).
