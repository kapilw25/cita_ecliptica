...
\item \textbf{CITA achieves 86.7\% instruction-alignment efficiency. How is this metric computed and why is it meaningful?}
...
\caption{CITA vs DPO improvement deltas across key benchmarks.}
...
DPO's wins (ECLIPTICA, Conditional Safety) reflect higher \textit{absolute} scores, but CITA achieves more \textit{consistent} improvement across diverse benchmarks. Consistency matters for deployment reliability.

\vspace{4mm}

%% Q2 - ATTACKS CORE BENCHMARK
\item \textbf{DPO\_Instruct (0.389) beats CITA\_Instruct (0.367) on ECLIPTICA---your own benchmark. Doesn't this undermine your entire paper?}
...
\textbf{Analogy}: A model that swings wildly between behaviors (high ECLIPTICA) may be less trustworthy than one that adapts consistently (moderate ECLIPTICA, high TruthfulQA). CITA optimizes for the latter.
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
    \item \textbf{4 external benchmarks}: TruthfulQA~\cite{lin2022truthfulqa}, Conditional Safety, Length Control, and AQI~\cite{hendrycks2023aligning} are independent, established benchmarks. CITA achieves highest instruction-alignment efficiency (86.7\%) when evaluated across all 5 benchmarks.
    ...
    \item \textbf{DPO beats CITA on ECLIPTICA}: If ECLIPTICA were biased toward CITA, DPO wouldn't win. This demonstrates benchmark fairness.
\end{enumerate}
...
\caption{Accuracy vs reward margin tradeoff between DPO and CITA.}
...
\caption{Four-stage CITA pipeline: each stage's purpose and consequence of omission.}
\label{tab:faq_pipeline_stages}
\end{table*}

\textbf{Ablation evidence}: Training CITA directly on base Llama (skipping SFT/DPO) produces 40\% lower ECLIPTICA scores. The staged approach is empirically necessary, not arbitrary complexity.
...
\bottomrule
\end{tabularx}
\caption{CITA failure modes, trigger conditions, and mitigations.}
...
\caption{CITA vs prompting baselines on ECLIPTICA and robustness.}
...
\caption{Ablation: effect of mandatory KL ($\lambda_{\text{KL}}$) on CITA performance.}
...
\caption{RLHF/PPO vs CITA comparison across training and deployment dimensions.}
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
    ...
    \item \textbf{Our benchmarks}: ECLIPTICA, TruthfulQA, Conditional Safety, Length Control, LITMUS---all measure \textbf{behavioral adaptation} to instructions, which is CITA's core claim.
\end{itemize}
...
\midrule
Training Code & \url{https://anonymous.4open.science/r/CITA_Anonymous-AC02} \\ \hline
...
CITA\_NoInstruct & \url{https://huggingface.co/anonymousML123/llama3-8b-pku-CITA-NoInstruct-DPO-NoInstruct} \\
\midrule
...
CITA\_Instruct & \url{https://huggingface.co/anonymousML123/llama3-8b-pku-CITA-Instruct-DPO-Instruct} \\
\midrule
Hyperparameters & Table~\ref{tab:hyperparams_exp} (exact values from Optuna) \\ \hline
Training Logs & TensorBoard logs in repository \\
\bottomrule
\end{tabularx}
\caption{Reproducibility artifacts and public URLs for CITA.}
...
\caption{DPO implicit vs CITA explicit KL: parameter coupling comparison.}
...
This decoupling is why CITA achieves 86.7\% instruction-alignment efficiency while DPO achieves only 56.1\%---a 30.6 percentage point (pp) gap explained entirely by KL architecture.
...
\item \textbf{Could CITA be ``cheating'' by learning an instruction-specific routing trick (a shallow prefix$\to$style map) rather than a genuinely switchable alignment policy? How do you rule out mere prompt-format overfitting?}
\begin{description}[leftmargin=1.2em,itemsep=3pt]
\item \textbf{We treat this as the key alternative hypothesis.} A weak explanation is that CITA maps a small instruction prefix to surface form (tone/verbosity) while leaving the underlying decision boundary (refuse vs comply; legal vs illegal; harm vs safe) mostly unchanged. Our evaluation therefore emphasizes tests that cannot be satisfied by stylistic compliance alone.
